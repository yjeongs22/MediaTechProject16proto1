const pages = ["homePage", "studentPage", "scheduleImagePage", "needsPage", "waitingPage", "resultPage", "adminPage", "adminCoursePage", "adminDetailPage"].filter(id => document.getElementById(id));

const pageTitles = {
  homePage: "홈",
  studentPage: "기본 정보 입력",
  scheduleImagePage: "시간표 이미지 분석",
  needsPage: "시간표 니즈 입력",
  waitingPage: "분석 대기",
  resultPage: "추천 결과",
  adminPage: "관리자 페이지",
...
let courseModalReturnTarget = null;
...
async function saveCoursesAsync(courses) {
  return setDataAsync("planpickCourses", courses);
}
...
async function showPage(pageId, pushHistory = true) {
  ...
  if (pageId === "adminCoursePage") renderCourseManager();
...
$("saveStudentBtn")?.addEventListener("click", async () => {
  ...
  showPage("scheduleImagePage");
});

function setImageAnalysisLoading(isLoading) {
  $("imageAnalysisStatus")?.classList.toggle("hidden", !isLoading);
  if ($("analyzeTimetableImageBtn")) $("analyzeTimetableImageBtn").disabled = isLoading;
  if ($("skipImageAnalysisBtn")) $("skipImageAnalysisBtn").disabled = isLoading;
}

function showTimetableImagePreview(file) {
  const preview = $("timetableImagePreview");
  if (!preview) return;

  if (!file) {
    preview.className = "image-preview empty";
    preview.innerHTML = "<p>이미지를 선택하면 이곳에 미리보기가 표시됩니다.</p>";
    return;
  }
  ...
  reader.onload = () => {
    ...
    preview.innerHTML = `<img src="${reader.result}" alt="업로드한 시간표 이미지 미리보기">`;
  };
  reader.readAsDataURL(file);
}

function runTimetableImageAnalysis() {
  if (!$("imageAnalysisStatus") || $("analyzeTimetableImageBtn")?.disabled) return;

  setImageAnalysisLoading(true);
  $("imageAnalysisTitle").textContent = "이미지를 분석하는 중입니다";
  $("imageAnalysisMessage").textContent = "시간표 정보를 확인하고 있습니다. 잠시만 기다려주세요.";

  setTimeout(() => {
    $("imageAnalysisTitle").textContent = "이미지 분석이 완료되었습니다";
    $("imageAnalysisMessage").textContent = "다음 화면에서 원하는 시간표 조건을 입력해주세요.";
    setTimeout(() => {
...
$("timetableImageInput")?.addEventListener("change", (event) => {
  showTimetableImagePreview(event.target.files?.[0]);
});

$("analyzeTimetableImageBtn")?.addEventListener("click", runTimetableImageAnalysis);
$("skipImageAnalysisBtn")?.addEventListener("click", () => showPage("needsPage"));

function setupEnrollmentAlertToggle() {
  const toggle = $("enrollmentAlertToggle");
  const status = $("enrollmentAlertStatus");
  if (!toggle || !status) return;

  const saved = localStorage.getItem("planpickEnrollmentAlert") === "on";
  toggle.checked = saved;
  status.textContent = saved ? "켜짐" : "꺼짐";

  toggle.addEventListener("change", () => {
    localStorage.setItem("planpickEnrollmentAlert", toggle.checked ? "on" : "off");
    status.textContent = toggle.checked ? "켜짐" : "꺼짐";
  });
}

function setupNeedTextCounter() {
  const input = $("needTextInput");
  if (!input) return;

  let counter = $("needTextCounter");
  if (!counter) {
    counter = document.createElement("span");
    counter.id = "needTextCounter";
    input.insertAdjacentElement("afterend", counter);
  }

  const update = () => {
    counter.textContent = `${input.value.length} / ${input.maxLength || 500}`;
  };
...
function setupUserDesignCopy() {
  if (appMode !== "user") return;

  const heroTitle = document.querySelector("#homePage .hero-panel h2");
  const heroDesc = document.querySelector("#homePage .hero-panel p");
  const heroBadge = document.querySelector("#homePage .hero-panel .badge");
  const heroButtons = document.querySelectorAll("#homePage .hero-actions button");
  const guideItems = document.querySelectorAll("#homePage .guide-strip div span");
  const needsTitle = document.querySelector("#needsPage .section-title");
  const needsDesc = document.querySelector("#needsPage .section-desc");
  const needsLabel = document.querySelector("#needsPage label");

  if (heroBadge) heroBadge.textContent = "SMART COURSE PLANNING";
  if (heroTitle) heroTitle.innerHTML = "더 스마트한<br><strong>수강신청,</strong><br>더 완벽한 학기";
  if (heroDesc) heroDesc.textContent = "AI가 당신의 졸업요건과 선호도를 분석하여 최적의 시간표를 추천해드려요.";
  if (heroButtons[0]) heroButtons[0].textContent = "최적 시간표 추천받기  →";
  if (heroButtons[1]) heroButtons[1].textContent = "수강신청 가이드  ◉";

  ["기본 정보 입력", "시간표 이미지 확인", "원하는 조건 작성", "추천 시간표 확인"].forEach((text, index) => {
    if (guideItems[index]) guideItems[index].textContent = text;
  });

  if (needsTitle) needsTitle.textContent = "원하는 시간표 조건";
  if (needsDesc) needsDesc.innerHTML = "<strong>원하는 조건을 문장으로 적어주세요.</strong><br>예: 오전 수업은 피하고 싶고, 금요일 공강이면 좋겠어요.";
  if (needsLabel) needsLabel.textContent = "자유입력";
  if ($("needTextInput")) $("needTextInput").placeholder = "예: 전공필수는 포함해주세요. 이동시간이 짧았으면 좋겠습니다.";
  if ($("submitRequestBtn")) $("submitRequestBtn").textContent = "AI 시간표 생성";
}

function setupExactDesignCopy() {
  if (appMode !== "user") return;
  ...
  if (needsDesc) needsDesc.innerHTML = "<strong>원하는 조건을 문장으로 적어주세요.</strong><br>예: 오전 수업은 피하고 싶고, 금요일 공강이면 좋겠어요.";
  if (needsLabel) needsLabel.textContent = "자유입력";
  if (needInput) needInput.placeholder = "예: 전공필수는 포함해주세요. 이동시간이 짧았으면 좋겠습니다.";
  if (submitButton) submitButton.textContent = "AI 시간표 생성";
}
...
function getGradeTypeValue(course) {
  const raw = String(course?.gradeType || course?.grading || "").toLowerCase();
  if (raw.includes("pass") || raw.includes("p/f") || raw.includes("pf")) return "passFail";
  return "graded";
}

function getGradeTypeText(course) {
  return getGradeTypeValue(course) === "passFail" ? "P/F 과목" : "학점 등급 과목";
}
...
function openRecommendedCoursesModal(courses) {
  ...
  courses.forEach(course => {
    ...
    button.innerHTML = `
      ...
      <span>${escapeHtml(course.type || "구분 없음")} · ${escapeHtml(getCourseScheduleText(course))} · ${Number(course.credit || 0)}학점 · ${escapeHtml(getGradeTypeText(course))}</span>
    `;
    button.addEventListener("click", () => {
      closeCourseListModal();
      openCourseModal(course, "recommendedCourses");
    });
...
샘플 과목 데이터:
${courses.map((course, index) => `${index + 1}. ${course.name} / ${course.type} / ${getCourseScheduleText(course)} / ${course.credit}학점 / ${getGradeTypeText(course)} / 강의실 ${course.room || "미입력"} / 강의코드 ${course.courseCode || "미입력"} / 난이도 ${formatDifficultyRating(course.difficulty)}/5 / 온라인 ${isOnlineCourse(course) ? "예" : "아니오"} / 팀플 ${course.team}`).join("\n")}
    ...
    courses.forEach(course => {
      ...
      label.innerHTML = `
        ...
        <div>
          ...
          <span>${course.type} · ${getCourseScheduleText(course)} · ${course.credit}학점 · ${getGradeTypeText(course)} · 난이도 ${formatDifficultyRating(course.difficulty)}/5 · ${course.room || "강의실 미입력"}</span>
        </div>
      `;
  ...
  courses.forEach((course, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="course-edit-grid">
        ...
        <div>
          <label>성적 방식</label>
          <select data-course-field="gradeType" data-course-index="${index}">
            <option value="graded" ${getGradeTypeValue(course) === "graded" ? "selected" : ""}>학점 등급 과목</option>
            <option value="passFail" ${getGradeTypeValue(course) === "passFail" ? "selected" : ""}>P/F 과목</option>
          </select>
        </div>
        <div>
          <label>강의실</label>
...
function collectCoursesFromManager() {
  ...
  document.querySelectorAll("[data-course-field]").forEach(input => {
    ...
    if (field === "gradeType") value = value === "passFail" ? "passFail" : "graded";
...
$("courseManager")?.addEventListener("click", async (event) => {
...
$("courseManager")?.addEventListener("change", async (event) => {
  if (!event.target?.dataset?.courseField) return;
  await saveCoursesAsync(collectCoursesFromManager());
  if (event.target.dataset.courseField === "online") renderCourseManager();
});

$("saveCoursesBtn")?.addEventListener("click", async () => {
  const courses = collectCoursesFromManager();

  const savedToFirebase = await saveCoursesAsync(courses);
  alert(savedToFirebase
    ? "강의 정보가 Firebase에 저장되었습니다. 다른 PC에서도 같은 강의 데이터를 불러올 수 있습니다."
    : "현재 PC에는 저장되었지만 Firebase 저장은 실패했습니다. Firestore Database와 보안 규칙 설정을 확인해주세요."
  );
  renderCourseManager();
});
...
$("resetCoursesBtn")?.addEventListener("click", async () => {
  if (confirm("강의 데이터를 샘플 상태로 되돌릴까요?")) {
    await saveCoursesAsync(defaultCourses);
    renderCourseManager();
  }
});

function ensureCourseModalBackButton() {
  const closeButton = $("closeModalBtn");
  if (!closeButton) return null;

  let backButton = $("modalBackBtn");
  if (backButton) return backButton;

  backButton = document.createElement("button");
  backButton.id = "modalBackBtn";
  backButton.type = "button";
  backButton.className = "secondary-btn small hidden";
  backButton.textContent = "뒤로가기";
  backButton.addEventListener("click", () => {
    $("courseModal")?.classList.add("hidden");
    if (courseModalReturnTarget === "recommendedCourses") {
      $("courseListModal")?.classList.remove("hidden");
    }
    courseModalReturnTarget = null;
...
function setCourseModalBackButtonVisible(visible) {
  const backButton = ensureCourseModalBackButton();
  if (!backButton) return;
  backButton.classList.toggle("hidden", !visible);
}

function ensureCourseModalGradeTypeField() {
  if ($("modalGradeType")) return;

  const grid = document.querySelector("#courseModal .detail-grid");
  if (!grid) return;

  const item = document.createElement("div");
  item.innerHTML = `
    <strong>성적 방식</strong>
    <p id="modalGradeType"></p>
  `;
  grid.appendChild(item);
}

function openCourseModal(course, returnTarget = null) {
  courseModalReturnTarget = returnTarget;
  ensureCourseModalGradeTypeField();
  setCourseModalBackButtonVisible(returnTarget === "recommendedCourses");
  ...
  $("modalTime").textContent = getCourseScheduleText(course);
  $("modalCredit").textContent = `${course.credit || 0}학점`;
  $("modalGradeType").textContent = getGradeTypeText(course);
...
function closeCourseModal() {
  courseModalReturnTarget = null;
  setCourseModalBackButtonVisible(false);
  $("courseModal").classList.add("hidden");
}
...
window.addEventListener("load", () => {
  setupUserDesignCopy();
  setupExactDesignCopy();
  applyRequestedDesignCopy();
  setupCreditSelector();
  setupEnrollmentAlertToggle();
  setupNeedTextCounter();
  getCourses();
  showPage(appMode === "admin" ? "adminPage" : "homePage", false);
  syncCloudDataAfterLoad();
});

function applyRequestedDesignCopy() {
  if (appMode !== "user") return;
  ...
  if (heroTitle) {
    heroTitle.innerHTML = [
      '<span class="hero-title-soft">더 스마트한</span>',
      "<strong>수강신청,</strong>",
      '<span class="hero-title-soft">더 완벽한 학기</span>'
    ].join("<br>");
  }
  if (heroDesc) {
    heroDesc.textContent = "AI가 당신의 졸업요건과 선호도를 분석하여 최적의 시간표를 추천해드려요.";
  }
  if (heroButtons[0]) heroButtons[0].textContent = "최적 시간표 추천받기  →";
  if (heroButtons[1]) heroButtons[1].textContent = "수강신청 가이드  ◉";

  ["홈", "추천 요청", "결과 확인"].forEach((text, index) => {
    if (navButtons[index]) navButtons[index].textContent = text;
  });

  if (pageTitle) {
    pageTitle.textContent = pageTitles[document.body.dataset.page] || "플랜픽";
  }
}

function applyAdminDesignCopy() {
  if (appMode !== "admin") return;

  const titleMap = {
    adminPage: "요청 관리",
    adminCoursePage: "강의 데이터 관리",
    adminDetailPage: "결과 입력"
  };
  const pageTitle = $("pageTitle");
  const currentPage = document.body.dataset.page;

  if (pageTitle && titleMap[currentPage]) {
    pageTitle.textContent = titleMap[currentPage];
  }

  ["요청 관리", "강의 데이터", "결과 입력"].forEach((text, index) => {
